...
namespace rootmap
{
    ...
    namespace Utility
    {
        ...
        {
            return (check_number == std::numeric_limits<double>::infinity());
        }
        ...
        double CalculateNewScale(double oldScale,
            short int oldH, short int oldV,
            short int newH, short int newV)
        {
            double oldDiagonal, newDiagonal;
            oldDiagonal = sqrt((double)((oldV * oldV) + (oldH * oldH)));
            newDiagonal = sqrt((double)((newV * newV) + (newH * newH)));
            return (double)(oldScale * (newDiagonal / oldDiagonal));
        }
        ...
        std::string CS_GetString(long string_resource_id)
        {
        ...
        bool CS_GetIndString(std::string& pstring, long strListID, long str_index)
        {
            RootMapLoggerInitialisation("rootmap.Utility");
            try
            {
        ...
        Dimension StringToDimension(char dim_string)
        {
            ...
            default:
            {
                std::string dimstr(1, dim_string);
        ...
        double StringToDouble(const char* c_string)
        {
            ...
            if (strlen(c_string) > 0)
            {
                if (c_string[0] == '-')
                {
                    ...
                    std::string str = std::string(c_string);
                    return -StringToDouble(str.substr(1, length - 1).c_str());
                }
                else {
        ...
        //  a non-zero integer
        //
        bool StringToBool(const std::string& s, bool throwOnFail)
        {
            std::string lowercasestr;
            for (std::string::const_iterator liter = s.begin();
                liter != s.end(); ++liter)
            {
...
#pragma error Need to re-implement for BigEndian!
        ...
        std::string ToString(bool b, BooleanStringRepresentation bsr)
        {
            switch (bsr)
            {
            case bsrYesNo:
        ...
        std::string ToString(double d, int precision, bool isFixed)
        {
            std::ostringstream s;
            if (isFixed)
            {
        ...
        SummaryRoutineGroup DetermineSummaryTypeArrayIndex(SummaryRoutine sr)
        {
            ...
            {
            case srRootLength:
                return summary_type_root_length_dependent;
            case srRootLengthDensity:
                return summary_type_root_length_dependent;
            case srRootTipCount:
                return summary_type_tip_count_dependent;
            case srRootTipCountDensity:
                return summary_type_tip_count_dependent;
            case srRootBranchCount:
                return summary_type_branch_count_dependent;
...
#pragma warning( pop )
        ...
        /**
         *    MSA NOTE: This method fingerprints the content of the file only (not its filename, timestamp or path).
         */
        SHA1Fingerprint GetFingerprint(const wxFileName& file)
        {
            SHA1Fingerprint f = SHA1Fingerprint();
            // MSA TODO Make this accept directories?
            if (!file.FileExists() || file.IsDir()) return f;
            wxFileName::SetCwd(file.GetPath());
            FILE* fp = __nullptr;
            if (fopen_s(&fp, file.GetFullName().c_str(), "rb") != 0)
            {
                return f;
            }
            if (!fp)
            ...
            unsigned int fingerprint[5];

            sha1.Result(fingerprint);

            f.zero = fingerprint[0];
            f.one = fingerprint[1];
            f.two = fingerprint[2];
            f.three = fingerprint[3];
            f.four = fingerprint[4];
        ...
        {
            RmSHA1 sha1;
            sha1.Input(str.c_str(), str.length());

            unsigned int fingerprint[5];

            sha1.Result(fingerprint);

            SHA1Fingerprint f(fingerprint);
